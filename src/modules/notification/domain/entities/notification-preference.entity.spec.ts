import { NotificationPreference, NotificationPreferenceProps } from './notification-preference.entity';
...
describe('NotificationPreference', () => {
  ...
  describe('create', () => {
    ...
    it('should throw NOTIFICATION_PREF_USER_ID_REQUIRED for empty userId', () => {
      expect(() => NotificationPreference.create('pref-1', { ...validProps, userId: '' })).toThrow(
        new DomainException('User ID is required', DomainErrorCode.NOTIFICATION_PREF_USER_ID_REQUIRED),
      );
    });

    it('should throw NOTIFICATION_PREF_ID_REQUIRED for empty id', () => {
      expect(() => NotificationPreference.create('', validProps)).toThrow(
        new DomainException('Notification preference ID is required', DomainErrorCode.NOTIFICATION_PREF_ID_REQUIRED),
      );
    });

    it('should throw NOTIFICATION_PREF_ID_TOO_LONG for id > 50 chars', () => {
      expect(() => NotificationPreference.create('a'.repeat(51), validProps)).toThrow(
        new DomainException('Notification preference ID cannot exceed 50 characters', DomainErrorCode.NOTIFICATION_PREF_ID_TOO_LONG),
      );
    });

    it('should throw NOTIFICATION_TYPE_REQUIRED for null type', () => {
      expect(() => NotificationPreference.create('pref-1', { ...validProps, type: null as any })).toThrow(
    ...
    it('should throw NOTIFICATION_CHANNEL_REQUIRED for null channel', () => {
      expect(() => NotificationPreference.create('pref-1', { ...validProps, channel: null as any })).toThrow(
    ...
    it('should throw NOTIFICATION_PREF_USER_ID_TOO_LONG for userId > 50 chars', () => {
      expect(() => NotificationPreference.create('pref-1', { ...validProps, userId: 'a'.repeat(51) })).toThrow(
        new DomainException('User ID cannot exceed 50 characters', DomainErrorCode.NOTIFICATION_PREF_USER_ID_TOO_LONG),
      );
    });
  ...
  describe('reconstitute', () => {
    it('should preserve all fields', () => {
      ...
      const pref = NotificationPreference.reconstitute('pref-1', {
        ...validProps,
        enabled: false,
      }, createdAt, updatedAt);
    ...
    it('should NOT emit events', () => {
      const pref = NotificationPreference.reconstitute('pref-1', validProps, new Date(), new Date());
      expect(pref.getDomainEvents()).toHaveLength(0);
    });
  });
  ...
  describe('updateEnabled', () => {
    it('should set enabled to true', () => {
      const pref = NotificationPreference.create('pref-1', { ...validProps, enabled: false });
      expect(pref.enabled).toBe(false);
