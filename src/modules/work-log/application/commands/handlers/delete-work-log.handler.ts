...
export class DeleteWorkLogHandler implements ICommandHandler<
...
> {
  ...
  async execute(
    ...
    if (command.userRole === 'manager') {
      // Manager: bypass ownership + locked check
      workLog.forceDelete(eventMetadata);
    } else {
      // Employee: ownership check + locked check
